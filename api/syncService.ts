...
export const getUnsyncedItems = async (
  ...
  try {
    ...
    console.error(`Error getting unsynced ${tableName}:`, error);
    return [];
  }
};
...
export const syncUnsyncedForTable = async (tableName: string) => {
  try {
    ...
    console.error(`Error in syncUnsynced for ${tableName}:`, error);
    return {
      success: false,
      message: `Sync failed for ${tableName} due to an error`,
      error,
  ...
  try {
    ...
    for (const serverItem of serverItems as ServerItem[]) {
      try {
        ...
        console.error(`Error processing server item for ${tableName}:`, error);
      }
    }
...
export const syncAllUnsynced = async () => {
  try {
    ...
    console.error("Error in syncAllUnsynced:", error);
...
export const pullAllChanges = async () => {
  try {
    ...
    console.error("Error in pullAllChanges:", error);
